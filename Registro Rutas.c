...
void agregarRuta(nodoDomicilio *aux,char *destino){
    ...
    scanf("%f",&distancia);
    printf("Ingrese el tipo de la ruta (terrestre,area o maritima): ");
    fflush(stdin);
    gets(tipoR);

    infoR ruta = {destino,tiempo,distancia,tipoR};
 ...
 */
void insertarRuta(GrafoPaseo *Paseo,int cantidadRutas){
    char *origen,*destino;
    nodoDomicilio *aux;
    ...
    for(i=Paseo->inicio;i!=NULL;i=i->siguiente){
        ...
        if(i->adyacencia->destinos == NULL){
            ...
            for(i1=i->adyacencia->destinos;i1!=NULL;i1=i1->siguiente){
                printf("Las rutas del domicilio son: \n\n");
...
void modificarTiempoRuta(GrafoPaseo *Paseo){
    ...
    while(1){
        printf("Ingrese el tipo de la ruta que se desea modificiar: ");
        gets(tipoRuta);
        if(strlen(tipoRuta)==0)
        {
            printf("Por favor ingrese un tipo de ruta");
        }
        else{
            break;
        }
 ...
 * El nombre del lugar de origen, destino y tipo de ruta debe ser de tipo char
 */
void eliminarRuta(GrafoPaseo *Paseo,int cantidadRutas){
    char *origen, *destino,*tipoRuta;
    origen = malloc(sizeof(char) * 40);
    destino = malloc(sizeof(char) * 40);
    tipoRuta = malloc(sizeof(char) * 40);
        ...
        if(strcmp(opcion1, "1") == 0)
        {
            ...
            while(1)
            {
                ...
                if(strcmp(opcion2, "1") == 0){
                    ...
                    insertarRuta(Paseo,cantidadRutas);
                }
                else if(strcmp(opcion2, "3") == 0){
                    while(1){
                    ...
                    /// funcion que elimina una ruta
                    eliminarRuta(Paseo,cantidadRutas);
                }
